...
const Lightbox = () => {
const { mediaArray, lightboxState, lightboxCloseHandler, handleNextPrevious, handleCloseOutside, lbContainer, lbPrevious, lbNext, lbClose } = useContext(LightboxContext);
  ...
  return (
    <div className='lightbox' ref={lbContainer} aria-hidden={!isOpen} onClick={handleCloseOutside}>
      <div className='lightbox__buttons'>
        {mediaArray.length > 1 && (
          <>
            <button className='button button--icon-only' ref={lbPrevious} onClick={() => handleNextPrevious(-1)}>
              <span className='icon icon-arrow-left' aria-label='Previous'></span>
            </button>

            <button className='button button--icon-only' ref={lbNext} onClick={() => handleNextPrevious(1)}>
              <span className='icon icon-arrow-right' aria-label='Next'></span>
            </button>
          </>
        )}

        <button className='button button--icon-only' ref={lbClose} onClick={lightboxCloseHandler}>
